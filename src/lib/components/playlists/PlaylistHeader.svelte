...
<div class="grid grid-cols-1 grid-rows-1">
  {#if isScrolled}
    <div
      ...
      {appearanceStore.disableBlur
        ...
        : 'bg-muted-foreground/10 dark:bg-muted/70 backdrop-blur-md border-input/15 p-2'}"
    >
      <Button
        variant="ghost"
        size="icon"
  ...
  {:else}
    <div
      ...
      style="--h: 5rem"
      class="relative row-start-1 col-start-1 flex-1 flex items-end bg-background"
    >
      ...
      {#key playlist.coverImage}
        <PlaylistCoverImage
          ...
          iconSize={64}
          youtubeIconSize={48}
          containerClass="shrink-0 overflow-hidden bg-muted relative grid place-items-center rounded-xl size-40 md:size-64"
          imageClass="w-full h-full object-cover relative z-10"
        />
      {/key}

      <div class="w-full flex flex-col gap-2 truncate h-40 md:h-64 ml-4">
        <Button
          variant="outline"
          size="icon"
