<script lang="ts">
  ...
  async function refreshPresets() {
    await playerStore.hydrateEqPresetsFromBackend().catch((error) => {
      console.error("Failed to refresh EQ presets:", error);
    });
  }
</script>

<PullToRefresh onStageOne={refreshPresets}>
  ...
  <div class="px-2 pb-4 pt-0.5 w-full space-y-2 mb-[50dvh]">
    ...
    >
      <p class="text-sm text-muted-foreground px-2 pb-1">
        Bypasses EQ, pre-amp, and analyzer processing while keeping master
                  ...
                  <div class="flex flex-row items-center gap-2 w-full">
                    ...
                    >
                      ...
                      {#if playerStore.equalizerBands.length > 1}
                        <button
                          onclick={() =>
                            playerStore.removeEqualizerBand(band.id)}
